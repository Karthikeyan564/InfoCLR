...
class SogCLR_Loss_mine(nn.Module):
    def __init__(self, dim_size, N=2900000, gamma=0.1, temperature=0.07, world_size=8, bsz=128, enable_surrogate=False, surrogate_c=1.0, cov_coeff=1.0):
        
        #Inputs:
        #   N is number of samples in training set
        
        super(SogCLR_Loss_mine, self).__init__()
        self.world_size = world_size
        self.s_I = torch.zeros(N).cuda()
        self.s_T = torch.zeros(N).cuda()
        self.b_I = torch.zeros(N).cuda()
    ...
    def forward(self, image_features, text_features, image_ids, text_ids, epoch):
        ...
        batch_size_vic = len(image_features)

        x = image_features - image_features.mean(dim=0)
        y = image_features - image_features.mean(dim=0)

        cov_x = (x.T @ x) / (batch_size_vic - 1)
        cov_y = (y.T @ y) / (batch_size_vic - 1)
        cov_loss = self.cov_coeff * off_diagonal(cov_x).pow_(2).sum().div(
            self.dim_size
        ...
        total_loss = image_loss.mean() + text_loss.mean() + cov_loss
...
class SogCLR_v2_mine(nn.Module):
    def __init__(self, dim_size, N=2900000, gamma=0.1, temperature=0.07, world_size=8, bsz=128, enable_surrogate=False, surrogate_c=1.0, std_coeff=5.0, cov_coeff=1.0):
        ...
        super(SogCLR_v2_mine, self).__init__()
        self.world_size = world_size
        self.s_I = torch.zeros(N).cuda()
        self.s_T = torch.zeros(N).cuda()
        self.b_I = torch.zeros(N).cuda()
    ...
    def forward(self, image_features, text_features, image_ids, text_ids, epoch):
        ...
        std_loss = self.std_coeff * torch.mean(F.relu(1 - std_x)) / 2 + torch.mean(F.relu(1 - std_y)) / 2  # variance term

        cov_x = (x.T @ x) / (batch_size_vic - 1)
        cov_y = (y.T @ y) / (batch_size_vic - 1)
        cov_loss = self.cov_coeff * (off_diagonal(cov_x).pow_(2).sum().div(self.dim_size) + off_diagonal(cov_y).pow_(2).sum().div(self.dim_size))  # covariance term
        ...
        total_loss = image_loss.mean() + text_loss.mean() + cov_loss + std_loss
...
# try to use temperature generator in place of individualized temperatures
class HybridSogCLR_Loss(nn.Module):
    def __init__(self, N=2900000, gamma=0.8, temperature=0.07,
                 bsz=128, world_size=8, eps=1e-8,
                 memory_momentum=0.9,
                 feature_dim=256,
        ...
        # Initialize adaptive temperature parameters
        self.use_adaptive_temp = use_adaptive_temp
        self.base_temperature = temperature
        self.current_temperature = temperature
        self.scale_momentum = 0.99  # For loss scaling
        ...
        # Register temperature buffer
        self.register_buffer('temperature', torch.tensor([temperature]))

        # Initialize running loss scales
        self.register_buffer('running_base_scale', torch.tensor(1.0))
        self.register_buffer('running_cov_scale', torch.tensor(1.0))
        self.register_buffer('running_diff_scale', torch.tensor(1.0))

        # Initialize memory banks
        self.memory_bank_size = min(N, 16384)
        self._initialize_memory_banks()

    def compute_covariance_loss(self, image_features, text_features):
        """Compute covariance loss with memory-efficient implementation"""
        def compute_cov(features):
            ...
            chunk_size = min(batch_size, 512)  # Smaller chunks for memory efficiency
            cov_diag = torch.zeros(features.size(1), device=features.device)

            for i in range(0, batch_size, chunk_size):
                chunk = features[i:min(i + chunk_size, batch_size)]
                cov_diag += (chunk.pow(2)).sum(0)

            cov_diag = cov_diag / (batch_size - 1)
            return cov_diag.mean()

        image_cov_loss = compute_cov(image_features)
        text_cov_loss = compute_cov(text_features)
        return image_cov_loss + text_cov_loss

    def compute_adaptive_temperature(self, sim_matrix):
        """Compute and update adaptive temperature"""
        with torch.no_grad():
            sim_std = sim_matrix.std()
            temp = self.base_temperature * (1 + torch.tanh(sim_std))

            # Check for temperature stability
            if torch.abs(temp - self.current_temperature) > 0.1 * self.base_temperature:
                self.warnings['temperature_warnings'] += 1

            self.current_temperature = temp
            return temp

    def update_memory_bank(self, features, ids, bank):
        """Safe memory bank update with improved error handling"""
        with torch.no_grad():
            try:
                # Move everything to CPU if memory issues occur
                ...
                valid_mask = (ids >= 0) & (ids < bank_size)

                if not torch.all(valid_mask):
                    warnings.warn(f"Invalid indices detected. Max index: {ids.max().item()}, Bank size: {bank_size}")
                    ids = ids[valid_mask]
                ...
                for i in range(0, len(ids), chunk_size):
                    ...
                    # Update bank
                    current_bank = bank[chunk_ids].clone()
                    updated_features = (
                        self.memory_momentum * current_bank +
                        (1 - self.memory_momentum) * chunk_features
                    )
                    bank.index_copy_(0, chunk_ids, updated_features)

            except Exception as e:
                warnings.warn(f"Memory bank update failed: {str(e)}")

    def compute_similarity_matrix(self, image_features, text_features):
        """Memory-efficient similarity computation with gradient checkpointing"""
        try:
            # Use smaller chunks
            chunk_size = 256  # Reduced from 512
            batch_size = image_features.size(0)

            # Pre-allocate output tensor on CPU if too large
            total_size = batch_size * text_features.size(0)
            if total_size > 1e7:  # Threshold for large matrices
                sim_matrix = torch.zeros(batch_size, text_features.size(0),
                                       device='cpu')
            else:
                sim_matrix = torch.zeros(batch_size, text_features.size(0),
            ...
            for i in range(0, batch_size, chunk_size):
                ...
                chunk_img = image_features[i:end_idx]

                # Use torch.matmul with gradient checkpointing if available
                if hasattr(torch, 'utils') and hasattr(torch.utils, 'checkpoint'):
                    chunk_sim = torch.utils.checkpoint.checkpoint(
                        lambda x, y: torch.matmul(x, y.T),
                        chunk_img, text_features
                    )
                else:
                    chunk_sim = torch.matmul(chunk_img, text_features.T)

                sim_matrix[i:end_idx] = chunk_sim.to(sim_matrix.device)

            # Apply temperature scaling
            temp = (self.compute_adaptive_temperature(sim_matrix)
                    if self.use_adaptive_temp else self.temperature)

            return sim_matrix.to(image_features.device) / temp

        except Exception as e:
            warnings.warn(f"Similarity computation failed: {str(e)}")
            return torch.zeros(image_features.size(0), text_features.size(0),
                             device=image_features.device)

    def __del__(self):
        """Cleanup method to handle memory properly"""
        try:
            # Clear memory banks
            if hasattr(self, 'b_I'):
                del self.b_I
    ...
    def forward(self, image_features, text_features, image_ids, text_ids, epoch):
        try:
            ...
            if not (image_features.isfinite().all() and text_features.isfinite().all()):
                raise ValueError("Input features contain inf or nan values")
            ...
            max_id = self.memory_bank_size - 1
            image_ids = torch.clamp(image_ids, 0, max_id)
            text_ids = torch.clamp(text_ids, 0, max_id)

            # Compute similarities
            with torch.cuda.amp.autocast(enabled=True):
                sim_matrix = self.compute_similarity_matrix(image_features, text_features)

                # Compute base contrastive loss
                base_loss, image_loss, text_loss = self.compute_base_loss(
                    sim_matrix, image_features, text_features, image_ids, text_ids, epoch)

                # Compute additional loss terms
                total_loss = base_loss

                if self.covariance_weight > 0:
                    cov_loss = self.compute_covariance_loss(image_features, text_features)
                    total_loss = total_loss + self.covariance_weight * cov_loss
                else:
                    cov_loss = torch.tensor(0.0, device=device)

                if self.difficulty_weight > 0:
                    diff_loss = self.compute_difficulty_loss(image_features, text_features, sim_matrix)
                    total_loss = total_loss + self.difficulty_weight * diff_loss
                else:
                    diff_loss = torch.tensor(0.0, device=device)

            # Update memory banks
            self.update_memory_bank(image_features.detach(), image_ids, self.b_I)
            self.update_memory_bank(text_features.detach(), text_ids, self.b_T)

            # Update loss scales
            self.update_loss_scales(base_loss, cov_loss, diff_loss)
            ...
            return total_loss, {
                ...
                'temperature': self.current_temperature if self.use_adaptive_temp else self.temperature.item(),
                'image_loss': image_loss.item(),
                'text_loss': text_loss.item(),
                'warnings': self.warnings,
                'memory_stats': {
                    'image_norm': torch.norm(self.b_I, dim=1).mean().item(),
                    'text_norm': torch.norm(self.b_T, dim=1).mean().item()
                }
            }
    ...
    def compute_base_loss(self, sim_matrix, image_features, text_features,
                          image_ids, text_ids, epoch):
        """Compute the base contrastive loss with SogCLR statistics"""
        diag_sim = torch.diagonal(sim_matrix)

        # Compute differences
        image_diffs = (sim_matrix - diag_sim[:, None]).clamp(min=-1e3, max=1e3)
        text_diffs = (sim_matrix - diag_sim[None, :]).clamp(min=-1e3, max=1e3)

        # Update statistics
        with torch.no_grad():
            exp_image_diffs = torch.exp(image_diffs).clamp(max=1e3)
            exp_text_diffs = torch.exp(text_diffs).clamp(max=1e3)

            g_I = torch.sum(exp_image_diffs, dim=1) / max(self.batch_size - 1, 1)
            g_T = torch.sum(exp_text_diffs, dim=0) / max(self.batch_size - 1, 1)
            ...
            if epoch == 0:
                ...
                s_I = (1 - self.gamma) * self.s_I[image_ids] + self.gamma * g_I
                s_T = (1 - self.gamma) * self.s_T[text_ids] + self.gamma * g_T

            self.s_I[image_ids] = s_I
            self.s_T[text_ids] = s_T

        # Compute weights
        weights_image = exp_image_diffs / (s_I.unsqueeze(1) + self.eps)
        weights_text = exp_text_diffs / (s_T.unsqueeze(0) + self.eps)

        # Compute losses
        image_loss = torch.mean(weights_image * image_diffs)
        text_loss = torch.mean(weights_text * text_diffs)

        # Combined loss
        base_loss = (image_loss + text_loss) / 2

        return base_loss, image_loss, text_loss

    def _initialize_memory_banks(self):
        """Initialize memory banks with proper size checks and error handling"""
        try:
            if self.memory_bank_size < self.min_bank_size:
                raise ValueError(f"Memory bank size {self.memory_bank_size} below minimum {self.min_bank_size}")

            self.register_buffer('b_I', torch.zeros(self.memory_bank_size, self.feature_dim))
            self.register_buffer('b_T', torch.zeros(self.memory_bank_size, self.feature_dim))
            self.register_buffer('s_I', torch.zeros(self.memory_bank_size))
            self.register_buffer('s_T', torch.zeros(self.memory_bank_size))

        except RuntimeError as e:
            self.warnings['memory_warnings'] += 1
            self.memory_bank_size = max(self.memory_bank_size // 2, self.min_bank_size)
            print(f"Reducing memory bank size to {self.memory_bank_size}")
            self._initialize_memory_banks()

    def compute_difficulty_loss(self, image_features, text_features, sim_matrix):
        """Compute difficulty-aware loss term with safety checks"""
        try:
            with torch.no_grad():
                # Estimate sample difficulties
                image_mean = image_features.mean(0, keepdim=True)
                text_mean = text_features.mean(0, keepdim=True)

                image_difficulties = torch.norm(
                    image_features - image_mean, dim=1)
                text_difficulties = torch.norm(
                    text_features - text_mean, dim=1)

                # Safe normalization
                image_weights = F.softmax(image_difficulties, dim=0)
                text_weights = F.softmax(text_difficulties, dim=0)

            # Weight the similarity matrix
            weighted_sim = sim_matrix * image_weights[:, None] * text_weights[None, :]

            return -torch.mean(weighted_sim)
        except Exception as e:
            print(f"Error in difficulty loss computation: {str(e)}")
            return torch.tensor(0.0, device=image_features.device, requires_grad=True)

    def _get_default_output(self):
        """Enhanced default output with memory stats"""
    ...
    #     Returns the image memory bank for backward compatibility."""
    #     return self.b_I

    def update_loss_scales(self, base_loss, cov_loss, diff_loss):
        """Update running averages of loss scales"""
        with torch.no_grad():
            self.running_base_scale = self.scale_momentum * self.running_base_scale + \
                                    (1 - self.scale_momentum) * base_loss.abs().mean()
            self.running_cov_scale = self.scale_momentum * self.running_cov_scale + \
                                   (1 - self.scale_momentum) * cov_loss.abs().mean()
            if isinstance(diff_loss, torch.Tensor):
                self.running_diff_scale = self.scale_momentum * self.running_diff_scale + \
                                        (1 - self.scale_momentum) * diff_loss.abs().mean()
    ...
    def normalize(self, features):
        ...
        with torch.no_grad():
            if self.running_mean is None:
                self.running_mean = features.mean(0)
                self.running_var = features.var(0, unbiased=False)
            else:
                batch_mean = features.mean(0)
                batch_var = features.var(0, unbiased=False)
                self.running_mean = self.momentum * self.running_mean + (1 - self.momentum) * batch_mean
                self.running_var = self.momentum * self.running_var + (1 - self.momentum) * batch_var

            return (features - self.running_mean) / (self.running_var + 1e-5).sqrt()
